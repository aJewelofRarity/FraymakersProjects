...
function getStageCharacters() {
	players = match.getCharacters();
	for (v in players) {
		match.createCustomGameObject("customHudObject", v);
	}
}
...
function initialize(){
	...
	self.pause();
	match.addEventListener(MatchEvent.INTROS_STARTED, getStageCharacters);
